...
def BindTelephoneRTP(UDP_IP_ADDRESS='192.168.0.102', UDP_PORT_NO=19000):
    ...
    while True:
        ...
        global Counter
        newdata = RTPBegingData  + hex(Counter)
        if Counter >= 255:
            Counter = 0
        Counter = Counter + 1
        newdata = fix_packet_size(newdata, 19)
        print newdata
        #print newdata
...
def dataReceivedFromSerial(data):
     ...
     if RingBeginData in data:
        ...
        global voiceData
        # with open("receive1.csv", "ab") as myfile:
        #     myfile.write(data)
            # myfile.write('\n')
        print (data, data[0], data[1], data[2], data[3], data[4])
        data = data[4:]
        global LasteCounter
        ReceivedCounter =  ord(data[0])
        #print ReceivedCounter
        #print LasteCounter
        if abs(LasteCounter- ReceivedCounter ) >= 0:
            print( LasteCounter  , data)
        LasteCounter =  ReceivedCounter
        voiceData = data
